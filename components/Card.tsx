...
export default function Card({ title, subtitle, status, onClick }: any) {
  return (
    <div 
      ...
      className="w-full py-6 pl-11 pr-6 bg-white border-b border-slate-100 hover:bg-[#BAC9E0] transition-all cursor-pointer group flex justify-between items-center box-border"
    >
      {/* Left Area: Name and Address */}
      <div className="flex-1 text-left">
        {/* Title: Parking Lot Name */}
        <h3 className="font-bold text-slate-800 group-hover:text-[#0067A3] transition-colors text-lg tracking-tight">
          {title}
        </h3>
        {/* Subtitle: Location Information (Displayed with icons) */}
        <p className="text-sm text-slate-400 mt-1.5 flex items-center gap-2 font-medium group-hover:text-slate-600">
